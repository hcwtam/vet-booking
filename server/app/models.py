...
class PetOwner(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.String(120), db.ForeignKey('user.uid'))
...
vet_animalType = db.Table('vet_animalType',
                          db.Column('vet_id', db.Integer,
                                    db.ForeignKey('vet.id')),
                          db.Column('animalType_id', db.Integer,
                                    db.ForeignKey('animal_type.id'))
                          )

vet_clinic = db.Table('vet_clinic',
                      db.Column('vet_id', db.Integer, db.ForeignKey('vet.id')),
                      db.Column('clinic_id', db.Integer,
...
class Vet(db.Model):
    ...
    specialties = db.relationship(
        'AnimalType', secondary=vet_animalType, backref='vets', lazy=True)
    clinic = db.relationship(
        'Clinic', secondary=vet_clinic, backref='vets', lazy=True)
...
class Clinic(db.Model):
    ...
    animal_types = db.relationship(
        'AnimalType', secondary=clinic_animalType, backref='clinics', lazy=True)
    ...
    opening_hours = db.relationship(
        'OpeningHours', backref='clinic', lazy=True)
    time_created = db.Column(db.DateTime(timezone=True),
                             server_default=db.func.now())
    time_updated = db.Column(db.DateTime(
        timezone=True), onupdate=db.func.now())
...
pet_illness = db.Table('pet_illness',
                       ...
                       db.Column('illness_id', db.Integer,
...
class Pet(db.Model):
    ...
    owner_id = db.Column(db.Integer, db.ForeignKey(
        'pet_owner.id'), nullable=False)
    illnesses = db.relationship(
        'Illness', secondary=pet_illness, backref='pets', lazy=True)
